...
# 附录 4（2026-08-27）：终版关账（v13 任务 3）

## 关账结论

**失衡定性：结构性，共三机制——两项已修、一项移交。**

| 机制 | 根因 | 处置 | 状态 |
|---|---|---|---|
| 装甲地板 | 无破甲塔对 9+ 甲重敌 floor 至 1 伤 | FlatShareCap=0.5（C.2），轻甲带不受影响，反甲教学保留 | ✅ 已修已测（TDArmorTests） |
| 编队截断 | SiegeDrill 优先表 #7 进不了 4 槽编队，配额谓词永假 | 配额换编队已落码，L20 3/3 实证进编队且被建造 | ✅ 已修已测 |
| B.4 转化停滞 | 建造循环在 3-4 塔 + 250-400 闲置预算处停滞 | 静态排查未定位，插桩侦查线保留 | 🔁 移交（代码会话插桩项） |

**帽后复跑终态（QA `28b403d`，3 种子 × L13/L20 × 5 变体 + 25 局门禁）**：L13 0/15（死于 5-9 波）、L20 0/15，种子稳定；底线门禁保持（可选塔 >1 时单塔种通关 = 0；L01/L05/L09 合计 15/15）。**判定：仍低于难度档。**

**终判：pre-expansion 内容面（8 塔）在 L10-L20 波次曲线下低于难度档属结构性预期，不再做扩充前逐关数值补丁——由内容扩充重织接管**（12 塔 + 4 Boss + 分支专精，批 3 波次重织）。扩充前继续调这 11 关的数值只会把重织基线搅浑。

**基线冻结**：`output/playtest/baseline_pre_expansion_20260825`（63 文件：digest + 门禁 CSV + 30 逐局 JSON）为扩充重织的对照档；自 `28b403d` 起 pre-expansion 平衡数值冻结，任何塔/敌/波数值改动都归入扩充重织批次提交。

**并发工件**：重织输入包 `design/spec/balance-reweave-input-v1.md`（L13 早期波经济结构 + 宽带逐关速读 + 批 4 难度档定义裁决）随本案关账同日落库。本案诊断线到此关闭；重开条件仅一个——B.4 插桩定位出建造循环的具体卡点（届时作为重织的代码侧输入，不是本案复活）。

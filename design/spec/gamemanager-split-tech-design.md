...
> 作者：代码会话（总导演 v7 计划任务 1）· 日期：2026-08-20 · 状态：设计会话排期相容性评审**通过**（2026-08-21，见文末评审节）
...
## 设计会话排期相容性评审（2026-08-21，v9 任务 3）

**结论：通过。** 排期逻辑（先纯/数据后行为、S5 前池化评估、R8 平衡优先让行）与对账表排期建议一致；三种解耦模式均有本项目已验证先例；R6/R7 把多会话与帧时序两大现实风险登记到位。**冻结期前置之二闭环。**

### 新增核对：两条新功能线与 S 阶段的隔离（v9 指定）

1. **worldmap v2 结构重做**（规格 `159bafd`，美术 ~30 张在途）与 **meta 升级系统**（规格 `meta-upgrade-system-spec-v1.md`）**双双触碰 `TDWorldMap`**——落地顺序固定：**worldmap v2 先行（画布），meta 入口挂接其上**（meta 规格的入口交互流已按 v2 布局编写：右下 DEPLOY 侧 `meta_entry_button`）。
2. **两条线都必须在拆分 S0 启动前落库**（并入风险登记为 **R9**）：
   - meta 的 `TDGameManager` 足迹仅结算桥一行级调用 + `TDCampaignProgression` 字段扩展（后者在"不拆"清单内，无冲突）
   - meta 面板用 M3 工厂族构建——先于 S4 落地则用现方法、后于 S4 则用静态门面转发，签名不变，两向兼容
   - 若任一线无法在冻结期前完成：**显式隔离**——该线冻结期间停摆，只在 `TDWorldMap`/新独立文件内留桩，不插 `TDGameManager` 改动（R6 认领约定适用）
3. 排期影响评估：S1-S7 内部顺序**不受影响**；新增的前置工作量 = worldmap v2 代码侧 + meta 三文件（估 3-4d），计入冻结期启动门槛。

### 评审补充（非阻塞建议）

- S0 基线三局建议包含一局 **L13 或 L20**（当前平衡敏感位），不只跑绿档关卡——帽后矩阵结果落地时基线应取其终版 HEAD
- M1 RunStats 随迁时保留 `_towerStats` 键语义（`AnalyticsId`）——meta 结算若需每局塔统计消费同一注册表，避免双份记账

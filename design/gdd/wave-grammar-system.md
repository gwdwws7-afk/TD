...
### Expansion Threat Vocabulary（批 2/3 按实现回写，2026-09-03）

六新敌进语法系统的方式（首现波纯净：降其他变量保证可识别）：

| 敌 | 首现 | 语法角色 | 波次占用（400 波实测） |
|---|---|---|---|
| 灰渣傀儡 | L04 | 击杀点位管理考（余烬堆 +25% 移速） | 33% |
| 轨面裂虫 | L05 | 地形利用考（直道 ×1.8/弯道 ×0.7） | 31% |
| 酸蚀孢囊 | L08 | 布位分散考（死亡塔位 debuff） | 25% |
| 锻炉骑兵 | L14 | 多段伤害克星（每波 3 免） | B/C 段 |
| 余烬行者 | L15 | 标记体系兑换目标 | C 段 |
| 回响虫群 | L19 | 清场秩序考（死亡复制） | C 段 |

**复现率重织达标**（矩阵 v2 §4 目标 → 实测）：疾行爬虫 89%→**47%**、灰烬虫群 91%→**39%**（目标 ≤55%）；每关平均原型 5.2→**9.2**（目标 ≥7.5）。
**boss_entry 波型**：四考试关 + 终局在 W20 插入相位 Boss（前两波 W18/19 各 -6% 预算垫付；prep ≥10s；相位阶梯单步下降语义见 enemy-ai GDD）。
**B 段奖励地板**：L10-15 波清奖励 ≥45（L13 的 W4-5 悬崖 37/39→45/45 为首例）；B 段曲线 = L09 × 1.04..1.24 线性（L13 定格 1.16，刀口裁定见 `design/reviews/l13-control-ruling-2026-09-03.md`）。

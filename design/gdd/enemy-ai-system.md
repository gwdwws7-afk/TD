...
### Expansion Archetype Behaviors（批 2 六敌，2026-09-02 按实现回写）

5. **灰渣傀儡（cinder_husk，L04｜65HP/1.0/2甲）**
   - 角色：击杀点管理考官
   - 行为：死亡留余烬堆 8s（半径 0.7），后续敌人踩过移速 ×1.25
   - 设计意图：惩罚乱杀位，教玩家控制击杀点

6. **轨面裂虫（rail_splitter，L05｜40HP/1.6）**
   - 角色：地形利用教学
   - 行为：路段方向点积 ≥0.9 判直道 ×1.8、弯道 ×0.7
   - 设计意图：与穿透线/弯道控场协教——直道是它的走廊

7. **酸蚀孢囊（acid_blister，L08｜78HP/0.9/5甲）**
   - 角色：首个打塔位的敌人
   - 行为：死亡喷酸 1.6 格内塔攻速 -10% ×4s（仅攻速因子）
   - 设计意图：教分散布位

8. **锻炉骑兵（forge_dragon→forge_dragoon，L14｜150HP/0.75/7甲）**
   - 角色：多段伤害克星
   - 行为：每波前 3 次命中免疫（逐波重置）
   - 设计意图：多段/灼烧/破甲反制，单体狙击吃亏

9. **余烬行者（ember_strider，L15｜95HP/2.0）**
   - 角色：标记体系的敌人侧搭档
   - 行为：被标记时受伤 ×1.25 且无法闪避（标记 = 必中）
   - 设计意图：Beacon/标记经济的兑换目标

10. **回响虫群（echo_brood，L18｜55HP/1.4）**
    - 角色：后期混乱源
    - 行为：死亡复制最近死亡非 Boss 敌 1 只（50% 血，出生在虫位）
    - 设计意图：与 L17 拟态主题呼应，惩罚无脑集杀

### Exam Boss Phases（批 2 四考试关 Boss，2026-09-02 按实现回写）

| Boss | 关卡/身份 | 相位阶梯（TDBossPhases） |
|---|---|---|
| 集装箱装卸兽 containermaw（320HP/0.5/10甲） | L05 储备窗口 | 50% 卸甲狂奔：甲→2、速 ×1.6；周期 12s 抛箱封锁建造格 10s |
| 岔道暴君 junction_tyrant（380HP/0.7/6甲） | L09 道岔承诺 | 周期 15s 强制改道；35% 分裂双体（余血折半、各走双线、双清 debuff，甲 4/速 0.85） |
| 窑炉看守 kiln_custodian（420HP/0.6/6甲） | L13 窑压时机 | 每 10s 叠 +1 甲（帽 +8）；唯 Kiln Purge 可削（-5 + 停 8s + 召唤 6 虫群/2 孢体） |
| 回响先驱 echo_harbinger（460HP/0.65/8甲） | L17 相位准备 | 每 6s 拟态玩家本场建造最多塔种：六族自效果（疾驰/燃云/滑流/重锻/弹幕/信号干扰）覆盖全 12 塔种；相位切换清自身 debuff |

相位阶梯语义：阈值单步下降（80%→20% 爆发也逐级触发每个过渡事件）。熔炉母体（L20）维持现状随矩阵重校。基线与行为逐值对表：`design/reviews/batch12-spec-audit-2026-09-02.md`。
